...
let get_transitions infoflows types ta =
...
let print_IFL_requirement_E rqr = ""
...
let print_NuSMV semantics oc =
  Printf.fprintf oc "%s\n" "MODULE main\n\n\n     DEFINE";
  print_attributes semantics.ta oc;

  Printf.fprintf oc "\n";
  print_types semantics.nodes oc;

  Printf.fprintf oc "\n";
  print_operations semantics.bigo oc;

  Printf.fprintf oc "\n";
  print_allows semantics.allows oc semantics.nodes semantics.ta;
...
let _ =
  ...
  try
    ...
    print_NuSMV semantics out_file
    )
  with 
    | Not_found -> (
