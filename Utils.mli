...
module StSL :
  sig
    type t = String.t * StringList.t
    val compare : String.t * StringList.t -> String.t * StringList.t -> int
  end

module StSLS :
  sig
    type elt = StSL.t
    type t = Stdlib__set.Make(StSL).t
    val empty : t
    val is_empty : t -> bool
    val mem : elt -> t -> bool
    val add : elt -> t -> t
...
module SLM :
  sig
    type key = StringList.t
    type 'a t = 'a Stdlib__map.Make(StringList).t
    val empty : 'a t
    val is_empty : 'a t -> bool
    val mem : key -> 'a t -> bool
    val add : key -> 'a -> 'a t -> 'a t
